...
tmux kill-session -t seedtest 2>/dev/null
tmux new-session -d -s seedtest -x 80 -y 24 -c /tmp
W=$(tmux list-windows -t seedtest -F '#{window_id}' | head -1)
...
APID=$(tmux display-message -p -t "$A" '#{pane_pid}')
BPID=$(tmux display-message -p -t "$B" '#{pane_pid}')
TABLE=$(mktemp); trap 'rm -f "$TABLE"; tmux kill-session -t seedtest 2>/dev/null' EXIT
...
[ -z "$(tmux show-options -wqv -t "$W" @claude-state)" ] && echo "ok   window falls back to idle" \
    || echo "FAIL window=$(tmux show-options -wqv -t "$W" @claude-state)"
